...
async function route(tagname) {
  const translated = await translate(tagname, {
    ...
    engine: "yandex",
    key:
      "trnsl.1.1.20200205T150624Z.0d3bdf6192dc75b0.bde2a5534a3961ab43ed5d789cc8afa76a31a39f"
  });
...
async function reply(ctx) {
  ...
  async function processingQueue(queue) {
    ...
    for (let i = 0; i < queue.length; i++) {
      let url = queue[i],
        illustId = url.link.split("/artworks/")[1],
        uid = url.user_id,
        illustDetails = await pixiv.illustDetail(illustId).catch(async err => {
      ...
      await replyIllust(ctx, illustDetails.illust, url.link, uid).catch(err => {
  ...
  async function startProcessing() {
    let queue = await memory.getNextQueue();
  ...
  async function replyIllust(ctx, illustDetails, url, uid) {
    ...
    let artistLink = "https://www.pixiv.net/en/users/" + illustDetails.user.id;
    let settings = await memory.getSettings(ctx.from.id);
    let tags = illustDetails.tags;
    // console.log(tags);
...
`;
    ...
    let photo_caption = await get_photo_caption();
    if (illustDetails.pageCount == 1) {
      // console.log(illustDetails)
      let original = illustDetails.metaSinglePage.originalImageUrl,
        filename = "full" + original.substr(original.lastIndexOf(".")),
        file = await pixivImg(original),
        photo = await pixivImg(illustDetails.imageUrls.large),
        file_caption =
      ...
      let metaPages = illustDetails.metaPages,
        mediaGroup = [],
        mediaGroupOriginal = [],
        photos = [],
        files = [],
